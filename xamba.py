...
def alternative_cumsum(input_tensor):
    """
    Alternative implementation of torch.cumsum using torch.matmul.
    """
    # Extract input tensor shape
    batch_size, num_groups, _, seq_len, _ = input_tensor.size()

    # Create a cumulative sum mask (lower triangular matrix)
    cumsum_mask = torch.tril(torch.ones(seq_len, seq_len, device=input_tensor.device))

    # Flatten the input along the last two dimensions
    flat_input = input_tensor.view(-1, seq_len, seq_len)  # Shape: [batch_size * num_groups, seq_len, seq_len]

    # Perform matrix multiplication to compute cumulative sum
    tensor_cumsum = torch.matmul(cumsum_mask, flat_input)  # Shape: [batch_size * num_groups, seq_len, seq_len]

    # Reshape back to original dimensions
    tensor_cumsum = tensor_cumsum.view(batch_size, num_groups, 1, seq_len, seq_len)

    return tensor_cumsum

################################# CumBA implementation #################################
def segment_sum_modified(input_tensor):
    """
    More stable segment sum calculation. Uses cumulative sums and masking instead of direct subtractions.
    """
    chunk_size = input_tensor.size(-1)
    ...
    input_tensor = input_tensor[..., None].expand(*input_tensor.size(), chunk_size)
    # Create a lower triangular mask with the diagonal set to 0
    mask = torch.tril(torch.ones(chunk_size, chunk_size, device=input_tensor.device, dtype=torch.bool), diagonal=-1)
    input_tensor = input_tensor.masked_fill(~mask, 0)
    # Compute actual cumsum
    tensor_segsum = alternative_cumsum(input_tensor)
    # Apply mask to keep only the lower triangular part of the cumulative sum result (including diagonal)
    mask = torch.tril(torch.ones(chunk_size, chunk_size, device=input_tensor.device, dtype=torch.bool), diagonal=0)
    tensor_segsum = tensor_segsum.masked_fill(~mask, -torch.inf)
    return tensor_segsum
################################# CumBA implementation #################################
...
class Mamba2Mixer(nn.Module):
    ...
    def torch_forward(self, input_states, cache_params: Optional[Mamba2Cache]=None, cache_position:Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None):
        ...
        if cache_params is not None and cache_params.seqlen_offset > 0:
            ...
            dt = nn.functional.relu(dt + self.dt_bias)  # XAMBA: SoftPlus to ReLU (ActiBA)
            ...
            # print('segment_sum shape 1: ', A.shape)
            # L = torch.exp(segment_sum(A))
            
            L = torch.exp(segment_sum_modified(A))  # XAMBA: CumBA, cumsum_1 changed to MatMul
            ...
            # print('segment_sum shape 2: ', nn.functional.pad(A_cumsum[:, :, :, -1], (1, 0)).shape)
            decay_chunk = torch.exp(segment_sum(nn.functional.pad(A_cumsum[:, :, :, -1], (1, 0))))

            states_permuted = states.permute(0, 2, 1, 3, 4)
            result = (decay_chunk[..., None, None] * states_permuted[:, :, None, ...]).sum(dim=2)
            # result = torch.randn(1, 128, 2, 64, 128)  ## AD: This was causing the problem for NPU broadcast Mul compilation
